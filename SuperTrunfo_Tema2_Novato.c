...
{  
    ...
    // COMPARAÇÃO ENTRE AS DUAS CARTAS VENCEDORAS

    if(SPC1 > SPC2) // COMPARAÇÃO COM MAIOR VALOR
    {
        printf("Resultado: Carta 1(%s) venceu com maior valor!\n", NdC1);
    }
    else
    {
        printf("Resultado: Carta 2 (%s) venceu com maior valor!\n", NdC2);
    }

    printf("Carta 1 (%s): %.2f\n", NdC1, CDP1);
    printf("Carta 2 (%s): %.2f\n", NdC2, CDP2);

    if(CDP1 < CDP2) // COMPARAÇÃO COM MENOR VALOR
    {
        printf("Resultado: Carta 1 (%s) venceu com menor valor!\n", NdC1);
    }
    else
    {
        printf("Resultado: Carta 2 (%s) venceu com menor valor!\n", NdC2);
    }

    // COMPARAÇÃO DE CARTA (PONTO TURÍSTICOS)

    printf("Comparação de Carta (Atributo: Pontos Turísticos)\n");
